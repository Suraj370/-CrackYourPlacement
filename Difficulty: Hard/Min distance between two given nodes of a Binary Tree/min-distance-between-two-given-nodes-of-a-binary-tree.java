...
class GfG {
    int findDist(Node root, int a, int b) {
        ...
        // Calculate distances from LCA to a and b
    ...
    // Helper method to find the distance from a node to a target value
    private int findDistance(Node node, int target, int distance) {
        if (node == null) {
            return -1;
        }
        ...
        int leftDist = findDistance(node.left, target, distance + 1);
        if (leftDist != -1) {
            return leftDist;
        }
